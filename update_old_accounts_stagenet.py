...
def createnewdbentry(userid):
    ...
    monero_walletcreate = MoneroWalletStagenet(user_id=userid,
                                               currentbalance=0,
                                               unconfirmed=0,
                                               address1='',
                                               address1status=1,
...
def checkforwork():
    the_users = User.query.order_by(User.id.desc()).all()
    for f in the_users:

        print("***********Start*********")
        # stagenet
        user_wallet = MoneroWalletStagenet.query \
            .filter(MoneroWalletStagenet.user_id == f.id) \
            .first()
